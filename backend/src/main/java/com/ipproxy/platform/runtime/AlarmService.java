...
public class AlarmService {
    ...
    private final RuntimeOperationsRepository db;private final ObjectMapper json;
    public AlarmService(RuntimeOperationsRepository db,ObjectMapper json){this.db=db;this.json=json;}
    ...
    public Map<String,Object> createRule(Map<String,Object> b,Long userId){
        ...
        try{Long id=db.createRule(code,name,type,metric,op,j(thresholdEnvelope(threshold)),duration,consecutive,severity,j(channels),notifyRecovery,repeat,userId);return decorateRule(need(db.rule(id),"ALARM_RULE_NOT_FOUND","规则不存在"));}
