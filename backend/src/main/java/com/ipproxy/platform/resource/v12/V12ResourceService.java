...
public class V12ResourceService {
    ...
    @Transactional public Map<String,Object> setRosHa(Long id,String role,Long replacementId,String switchStatus,String reason,UserPrincipal actor,String sourceIp){
        ros(id);
        String r=n(role).isBlank()?"STANDALONE":role.trim().toUpperCase(Locale.ROOT);
        ...
        String sw=n(switchStatus).isBlank()?"NORMAL":switchStatus.trim().toUpperCase(Locale.ROOT);
        db.updateRosHa(id,r,replacementId,sw,actor.userId());
        audit.success(actor,"RESOURCE","ROS",id,"UPDATE_HA",required(reason,"操作原因不能为空"),sourceIp);
        Map<String,Object> out=new LinkedHashMap<>();out.put("id",id);out.put("haRole",r);out.put("replacementRosId",replacementId);out.put("switchStatus",sw);out.put("upstreamControl","PENDING_ADAPTER");return out;
    }
