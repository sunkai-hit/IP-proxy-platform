...
function roleIds(row:any){return String(row.role_ids||'').split(',').filter(Boolean).map(Number)}
function newUser(){editId.value=null;copyForm(userForm,{status:'ACTIVE',roleIds:[]});userDialog.value=true}
function editUser(row:any){editId.value=row.id;copyForm(userForm,{...row,displayName:row.display_name,roleIds:roleIds(row)});userDialog.value=true}
...
function editRole(row:any){if(row.built_in)return;editId.value=row.id;copyForm(roleForm,{roleCode:row.role_code,roleName:row.role_name,dataScope:row.data_scope,description:row.description,status:row.status});roleDialog.value=true}
...
async function saveRolePermissions(){if(!selectedRole.value||selectedRole.value.built_in)return;await systemApi.saveRolePermissions(selectedRole.value.id,checkedPermissionIds.value);ElMessage.success('角色权限已保存');await loadRoles()}
    ...
    <el-card shadow="never">
      <el-tabs v-model="tab">
        ...
        <el-tab-pane label="角色与权限" name="roles">
          <div class="split">
            <div><div class="toolbar"><el-button type="primary" @click="newRole">新增角色</el-button></div><el-table :data="roles" highlight-current-row @current-change="selectRole"><el-table-column prop="role_code" label="编码"/><el-table-column prop="role_name" label="角色"><template #default="s"><span>{{ s.row.role_name }}</span><el-tag v-if="s.row.built_in" size="small" style="margin-left:6px">内置</el-tag></template></el-table-column><el-table-column prop="permission_count" label="权限数" width="90"/><el-table-column label="操作" width="80"><template #default="s"><el-button link :disabled="s.row.built_in" @click.stop="editRole(s.row)">编辑</el-button></template></el-table-column></el-table></div>
            <div class="permission-panel"><h3>{{ selectedRole ? `授权：${selectedRole.role_name}` : '选择左侧角色进行授权' }}</h3><el-alert v-if="selectedRole?.built_in" title="超级管理员为系统内置角色，默认拥有全部系统权限，不允许在管理端修改。" type="warning" :closable="false"/><el-checkbox-group v-model="checkedPermissionIds" :disabled="selectedRole?.built_in"><el-checkbox v-for="p in permissions" :key="p.id" :value="p.id">{{ p.permission_name }} <small>{{ p.permission_code }}</small></el-checkbox></el-checkbox-group><el-button v-if="selectedRole&&!selectedRole.built_in" type="primary" @click="saveRolePermissions">保存授权</el-button></div>
          </div>
        </el-tab-pane>
